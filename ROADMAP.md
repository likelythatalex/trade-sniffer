...
| **Failed→revived event study (MFE)** | DONE (tooling; data-gated for results) | `src/backtest/event_study.py` (`python -m src.backtest.event_study`). Path-dependent: reconstructs episodes (`episodes.py`) from the live `signals.csv`, measures each one's close-path **MFE/MAE + time-to-target**, and compares the *revived* cohort vs *first-time* flags. Offline + **point-in-time (no survivorship bias)** — the honest counterpart to replay; no fetch, no schema change. Limits: excursions are **close-based** (under-state true high/low MFE) and it's **data-gated** (needs accrued fail→revive history). Kept separate from the core IC harness (no `transition` conditioning there → no selection bias). True-OHLC, plan-resolved outcomes are the separate "plan-outcome simulator" below. |
| **Plan-outcome simulator + policy sweep** | DONE | `src/backtest/plan_sim.py` (`python -m src.backtest.plan_sim`). (1) The path-dependent evaluator already existed — `trade_outcome.evaluate_outcome` (stop-vs-target-first → realized R, conservative tie-break). (2) `collect_trials` replays history once (via the extracted `replay.score_history` generator) to capture each directional signal's `Levels` + forward OHLC; `simulate` runs `plan_trade`→`evaluate_outcome` per `TradePlanConfig`; `sweep` runs the `stop_method`×`max_stop_pct` grid over the same trials and ranks by expectancy-R. **Faithful entry:** models the breakout trigger (a non-triggering breakout is `no_fill`, not a fabricated loss). **Discipline baked in:** `--oos-frac` holds out a date-tail for the anti-overfit check, `--step` thins overlapping samples, and the report's caveat says replay is survivorship-biased + in-sample → prefer plateaus over peaks, use the live journal for absolute expectancy. Pure-on-config `plan_trade` is what makes the A/B fair. |
...
| **Dashboard: toggle to view recently failed setups** | DONE | Collapsed "Recently invalidated" list in the sidebar (built from this run's `failed` transitions): each shows the prior direction/score → the current score it fell to (`n/a` if not re-evaluated this run). Informational, not clickable candidates. `scanner._failed_setups` + a `failed` param on `render_dashboard`. **Episode/transition history (DONE):** `episodes.py` reconstructs prior episodes from `signals.csv`; a re-flagged setup now carries a `↻N` badge + history line on the dashboard and is fed that history in the reviewer prompt (regenerating its stale prior-episode review once/day). |
| **Dashboard: enlarge the chart** | DONE | Chart height is now viewport-based (`72vh`, min 420px) so it's the dominant area, plus a **Fullscreen** toggle (Fullscreen API; `autoSize` resizes the chart via its ResizeObserver). Template/CSS only. |
| **Dashboard: client-side search/filter/sort** | DONE | Toolbar in the candidate list: a ticker filter box + sort (score ↓/↑, ticker A–Z), reordering the same row elements in place (click handlers survive) with live counts + a "no matches" note. Pure template/JS — **no backend** (all candidate data is already embedded in the page). Universe add/remove + journal CRUD are *mutations* → Hosting Phase B, not this. |
| **Dashboard: mobile-responsive layout** | DONE | `@media (max-width: 760px)` stacks the layout and **hides the chart/legend/tools while keeping everything else** (list, scores, reasons, plan, review); the "open in TV ↗" link covers charting on mobile. Chart drawing is extracted to `drawChart()` and guarded by `chartVisible()` so the hidden chart isn't created as a zero-size canvas. Template/CSS only. |
