...
| **Tune data-quality to not exclude real moves** | TODO | The spike (`range >> ATR`) and split-gap heuristics can't tell a violent-but-real move from a glitch, so ~10 liquid names drop every run. Signature: real moves come on **huge volume**, glitches don't → add a volume discriminator; the split-gap check is also largely obsolete under `auto_adjust=True`. Touches the conservative `data_quality` core — needs a design call. |
