...
| **Dashboard: toggle to view recently failed setups** | DONE | Collapsed "Recently invalidated" list in the sidebar (built from this run's `failed` transitions): each shows the prior direction/score → the current score it fell to (`n/a` if not re-evaluated this run). Informational, not clickable candidates. `scanner._failed_setups` + a `failed` param on `render_dashboard`. **Still TODO (separate):** feed the reviewer the setup's `transition`/episode history so a re-flagged setup is reviewed in context (the agent currently sees only the as-of snapshot). |
